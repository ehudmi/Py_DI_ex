...
# The user will be able to play again and again. Once the user decides to exit the program, we will print a
# summary of the outcomes of all the games: how many times they won, lost or and tied the computer.
...
# game.py – this will contain a Game class which will have functions to play a single game of rock-paper-scissors
...
# get_user_item(self) – Ask the user to select an item (rock/paper/scissors). Keep asking until the user has
# selected one of the items – use data validation and looping. Return the item at the end of the function.
...
# Return either win, draw, or loss. Where win means that the user has won, draw means the user and the computer
...
# print_results(results) – this should print the results of the games played. It should have a single parameter
# named results; which will be a dictionary of the results of the games played.
# It should display these results in a user-friendly way, and thank the user for playing.


# Note: results should be in this form: {win: 2,loss: 4,draw: 3}. Bear in mind that this dictionary will need
# to be created and populated in some other part of our code, and passed in to the print_results function at
# the right time.
...
# Create a new Game object (see below), and call its play() function, receiving the result of the game that
# is returned.
# Remember the results of every game that is played.

# When the user chooses to exit the program, call the print_results function in order to display a summary of
